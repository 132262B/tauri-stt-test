...
/// 환경변수 SELF_WIN_SEC 로 조정 가능(기본 12s). 큰 모델은 짧게 잡아 재전사 비용을 낮춤.
fn target_window() -> usize {
    std::env::var("SELF_WIN_SEC").ok().and_then(|v| v.parse().ok()).unwrap_or(12usize) * 16_000
}
/// 무음이 안 와도 이 길이에서는 강제 리셋. target 의 ~1.6배.
fn max_window() -> usize {
    target_window() * 5 / 3
}
/// 리셋 가능한 "무음" 판정 RMS 임계.
