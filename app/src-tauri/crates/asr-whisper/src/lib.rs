...
pub struct WhisperRsBackend {
    ...
    /// 전사용 state 를 캐시(재사용). 매 전사마다 새 state 를 만들면 CoreML 인코더가
    /// 매번 다시 로드돼 치명적으로 느려지므로, state 를 한 번만 만들어 재사용한다.
    state: std::sync::Mutex<Option<whisper_rs::WhisperState>>,
}
...
impl WhisperRsBackend {
    pub fn load(model_path: impl AsRef<Path>, language: Option<String>) -> Result<Self, String> {
        ...
        Ok(Self { ctx, language, state: std::sync::Mutex::new(None) })
    }
    ...
    pub fn transcribe(&self, audio: &[f32], init_prompt: &str) -> Result<Vec<WhisperToken>, String> {
        let mut guard = self.state.lock().map_err(|_| "state 잠금 실패".to_string())?;
        if guard.is_none() {
            *guard = Some(
                self.ctx
                    .create_state()
