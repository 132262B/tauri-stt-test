...
//! Python 0. 실행: `cargo test -p diar-campplus --test e2e_diarized -- --ignored --nocapture`
...
use diar_campplus::OnlineDiarizer;
use tokio::sync::mpsc;
...
async fn two_speakers_get_distinct_labels() {
    ...
    let backend: Box<dyn StreamingAsrBackend> =
        asr_whisper::self_streaming_backend(base.join("models/ggml"));
