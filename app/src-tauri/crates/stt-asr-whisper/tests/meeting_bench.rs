...
//! 실행: `cargo test -p stt-asr-whisper --test meeting_bench -- --ignored --nocapture`

use std::path::PathBuf;
use std::time::Instant;

use stt_asr_whisper::WhisperSelfBackend;
use stt_core::asr::{AsrConfig, SelfStreamingProcessor, StreamingAsrBackend};
...
async fn meeting_streaming_bench() {
    ...
    for i in 0..n_sec {
        let seg = &audio[i * 16_000..(i + 1) * 16_000];
        proc.insert_audio_chunk(seg, (i + 1) as f64);
        let t0 = Instant::now();
        ...
        for tk in &toks {
            ...
            // 확정 지연 = 현재 입력시각 - 토큰 끝시각(근사)
            commit_lag_sum += (i + 1) as f64 - tk.end;
            commit_events += 1;
        }
    }
    ...
    eprintln!("틱당 최대 {:.0}ms, 평균 {:.0}ms", max_ms, total_ms / n_sec as f64);
    eprintln!("실시간 미추종 틱(>1000ms): {over_rt}/{n_sec}");
    if commit_events > 0 {
