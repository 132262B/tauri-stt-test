...
pub fn start_file(tx: Sender<AudioFrame>, path: PathBuf) -> Result<CaptureControl, String> {
    let mut reader = hound::WavReader::open(&path).map_err(|e| format!("WAV 열기 실패({path:?}): {e}"))?;
    ...
    eprintln!("[capture] 파일 입력 '{}' {:.1}s ({} samples @16k)", path.display(), samples.len() as f64 / 16_000.0, samples.len());
    ...
    thread::spawn(move || {
        ...
        while i < samples.len() {
            ...
            t += dur;
            if tx
                .send(AudioFrame { samples: seg, t_start: t0, t_end: t, source: AudioSource::Mic })
                .is_err()
            {
                break;
            }
